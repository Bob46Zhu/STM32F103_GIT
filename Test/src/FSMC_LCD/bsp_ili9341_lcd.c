...
static uint16_t CurrentBackColor = WHILTE;	//背景色
...
static void ILI9341_FSMC_Config(void)
{
	...
	//地址建立时间(ADDSET)为1个HCKL 2/72M = 28ns
...
static void ILI9341_REG_Config(void)
{
	...
	/* Frame Rate Control (In Normal/Full Colors) (B1h) */
	DEBUG_DELAY();
	...
	ILI9341_Write_Data(0xC8);	//竖屏 左上角到(起点)到右下角(终点)
	DEBUG_DELAY();
...
/*
 * ILI9341初始化函数，如果要用到LCD，一定要用调用这个函数
 * */
 ...
 *	其中0、2、4、6模式的X方向像素为240，Y方向像素为320
 *	其中1、3、5、6模式的X方向像素为320，Y方向像素为240
 *
 *
...
void ILI9341_OpenWindow(uint16_t usX,uint16_t usY,uint16_t usWidth,uint16_t usHeight)
{
	...
	ILI9341_Write_Data(usX>>8);	//先高8位，后低8位
	ILI9341_Write_Data(usX & 0xFF); //设置起始点和结束点
	ILI9341_Write_Data((usX + usWidth -1) >> 8);
	ILI9341_Write_Data((usX + usWidth -1) & 0xFF );

	ILI9341_Write_Cmd(CMD_SetCoordinateY); //设置Y坐标
	ILI9341_Write_Data(usY>>8);	//先高8位，后低8位
	ILI9341_Write_Data(usY & 0xFF); //设置起始点和结束点
 ...
 * usWidth：窗口的宽度
 * usHeight：窗口的高度
 * 可使用：LCD_SetBackColor、LCD_SetTextColor、LCD_SetColors函数设置颜色
 * */
 ...
 * usX：在特定扫描方向下该点的X坐标
 * usY：在特定扫描方向下该点的Y坐标
 * 可以使用LCD_SetBackColor、LCD_SetTextColor、LCD_SetColors函数设置颜色
 * */
...
{
	uint8_t byteCount,bitCount,fontLenght;
	...
	//每个字模的字节数
	fontLenght = (LCD_Currentfonts->Width * LCD_Currentfonts->Height)/8;
	//字模首地址
	//ascii码表偏移值乘以每个字模的字节数，求出字模的偏移位置
	Pfont = (uint8_t *)&LCD_Currentfonts->table[ucRelativePosition * fontLenght];
	//设置显示窗口
	...
	for(byteCount = 0;byteCount < fontLenght;byteCount++)
	{
		//一位一位处理要显示的颜色
 ...
 * 		本参数可以使用宏LINE(0)、LINE(1)等方式指定文字坐标
 * 		宏LINE(x)会根据当前选择的字体来计算Y坐标值
 * 		显示中文且使用LINE宏时，需要把英文字体设置成Font8x16
 * Pstr：要显示的英文字符串的首地址
 * 可使用LCD_SetBackCorlor、LCD——SetTextColor、LCD_SetColors函数设置颜色
 * */
...
/*
 * 在ILI9341显示器上显示英文字符串（沿Y轴方向）
 * usX：在特定扫描方向下字符的起始X坐标
 * usY：在特定扫描方向下字符的起始Y坐标
 * pStr：要显示的英文字符串的首地址
 * 可使用LCD_SetBackColor、LCD_SetTextColor、LCD_SetColor函数设置颜色
 * */
 ...
 * TextColor：用来存储前景(字体)颜色的指针变量
 * BackColor：用来存储背景颜色的指针变量
 * @retval None
 * */
